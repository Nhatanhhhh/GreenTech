...
namespace BLL.Service.Category
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<IEnumerable<CategoryDTO>> GetWithQueryAsync(
        ...
        public async Task<CategoryDTO> UpdateAsync(int id, UpdateCategoryDTO updateDto)
        {
            ...
            if (updateDto.ParentId.HasValue)
            {
                ...
                if (
                    await _categoryRepository.IsCircularReferenceAsync(id, updateDto.ParentId.Value)
                )
                    throw new ArgumentException(
                        "Không thể đặt danh mục con làm danh mục cha (tham chiếu vòng)"
                    );
            }
