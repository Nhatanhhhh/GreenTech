...
namespace DAL.Repositories
{
    public class BlogRepository : IBlogRepository
    {
        ...
        public async Task<IEnumerable<Blog>> GetAllAsync(BlogQueryParamsDTO queryParams)
        {
        ...
        private IQueryable<Blog> ApplyFilters(IQueryable<Blog> query, BlogQueryParamsDTO queryParams)
        {
            if (!string.IsNullOrWhiteSpace(queryParams.SearchTerm))
            {
                var searchTermLower = queryParams.SearchTerm.ToLower();
                query = query.Where(b => b.Title.ToLower().Contains(searchTermLower) || b.Content.ToLower().Contains(searchTermLower));
            }
        ...
        private IQueryable<Blog> ApplySorting(IQueryable<Blog> query, BlogQueryParamsDTO queryParams)
        {
            ...
            switch (queryParams.SortBy?.ToLower())
            {
                case "title":
                    ...
                    break;
                case "publishedat":
                    query = isDescending ? query.OrderByDescending(b => b.PublishedAt) : query.OrderBy(b => b.PublishedAt);
                    break;
                case "viewcount":
                    query = isDescending ? query.OrderByDescending(b => b.ViewCount) : query.OrderBy(b => b.ViewCount);
                    break;
                case "createdat":
                default:
                    query = isDescending ? query.OrderByDescending(b => b.CreatedAt) : query.OrderBy(b => b.CreatedAt);
                    break;
            }
            return query;
        }
